...
def get_bin_indexes(annotations, bin_information):
...
def styling_bin_table(bin_colors, annotation_colors, unique_bin_annotations):
    ...
    for col in columns:
        numeric_df = bin_information_display[[col]].select_dtypes(include=[np.number])
    ...
    for bin in bin_information_display['Bin']:
...
def get_bin_and_annotation_colors(cyto_elements, bin_information):
    bin_colors = {}
    annotation_colors = {}
...
def annotation_visualization(bin_information, unique_bin_annotations, bin_matrix):
    G = nx.Graph()
    ...
    for i, annotation_i in enumerate(unique_bin_annotations):
        for j, annotation_j in enumerate(unique_bin_annotations[i + 1:], start=i + 1):
...
bin_information_columns = ['Bin','Domain','Kingdom','Phylum','Class','Order','Family','Genus','Species','Restriction sites','Length','Coverage','Self contact']

# Add a "Visibility" column to the bin_information_display DataFrame
...
bin_colors, annotation_colors = get_bin_and_annotation_colors([], bin_information)
...
def update_visualization(reset_clicks, confirm_clicks, visualization_type, selected_annotation, selected_bin, table_data):
    ...
    if triggered_id == 'reset-btn':
        ...
        if visualization_type == 'taxonomy_hierarchy':
            ...
            cyto_elements, bar_fig = annotation_visualization(bin_information, unique_bin_annotations, bin_matrix)
            ...
            try:
                cyto_elements, bar_fig = contig_visualization(selected_annotation, selected_bin, bin_information, unique_bin_annotations)
            except Exception as e:
                logger.error(f'Error in contig_visualization: {e}')
                cyto_elements = []
                bar_fig = go.Figure()
...
def update_dropdowns(selected_annotation, visualization_type):
    ...
    if visualization_type == 'bin' and selected_annotation:
        bin_index_dict = get_bin_indexes(selected_annotation, bin_information)
